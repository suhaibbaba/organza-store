 ...
 *     correcting to another, with the direction flipping under the user.
 *   - the service worker empties its shell cache on every deploy
 *     (public/sw.js, `activate`). A preference kept in there would be wiped
 *     by the update that installs the new build — which is exactly when it
 *     was going missing.
 *   - it survives the app being closed, which a session cookie does not.
 *     next-intl's own default carries no max-age, so on a phone the choice
 *     lasted until the app was swiped away and no further.
 *
 ...
 * soft navigation, and from the middleware on a hard one. The POS uses the
...
/** A year. Long enough that "remembered" means what staff would expect it to. */
export const LOCALE_COOKIE_MAX_AGE_SECONDS = 60 * 60 * 24 * 365;

/**
 * The whole app, and not the directory the cookie happened to be set from.
 *
 * Without this, a Set-Cookie sent while answering /ar/products defaults to
 * that path — so the launch that matters most, the installed app opening on
