...
import type { Product, Region, SalesChannel } from '@/types'
...
async function request<T>(
  ...
  // Store API requires a publishable API key
  if (!useAdmin && settings.publishableKey) {
    headers['x-publishable-api-key'] = settings.publishableKey
  }

  const res = await fetch(`${settings.backendUrl}${path}`, { ...rest, headers })
...
export async function getRegions(): Promise<Region[]> {
  const data = await request<{ regions: Region[] }>('/admin/regions?limit=20&fields=id,name,currency_code,countries.iso_2')
  return data.regions
}

export async function getSalesChannels(): Promise<SalesChannel[]> {
  const data = await request<{ sales_channels: SalesChannel[] }>('/admin/sales-channels?limit=20&fields=id,name')
  return data.sales_channels
}

// ── PRODUCT LOOKUP (Store API — returns calculated_price) ──
//   Needs a publishable API key + region_id for pricing.
function buildProductQuery(q: string, regionId: string | null): string {
  // Field syntax per Medusa v2 docs:
  //   *variants.calculated_price   → include variant + calculated price object
  const params = new URLSearchParams()
  if (q) params.set('q', q)
  params.set('limit', '20')
  params.set('fields', '*variants,*variants.calculated_price,thumbnail')
  if (regionId) params.set('region_id', regionId)
  return params.toString()
}

/** Find by exact barcode OR SKU match. Returns null if nothing matches. */
export async function findByCode(code: string): Promise<{ product: Product; variant: Product['variants'][number] } | null> {
  const settings = loadSettings()
  ...
  const data = await request<{ products: Product[] }>(`/store/products?${qs}`, { useAdmin: false })
...
export async function createOrder(input: CreateOrderInput): Promise<{ id: string; display_id?: number }> {
  const draftBody = {
  ...
  const completed = await request<{ order: { id: string; display_id?: number } }>(
