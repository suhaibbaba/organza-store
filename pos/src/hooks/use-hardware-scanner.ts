...
export function useHardwareScanner({ enabled, onScan }: UseHardwareScannerOptions) {
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (HARDWARE_SCAN_TERMINATORS.includes(event.key)) {
        const code = buffer;
        buffer = "";
      ...
      // Anything that isn't a single character (arrows, Escape, F-keys) is
      // not part of a code, and ends whatever was being collected.
      ...
      buffer = gap > HARDWARE_SCAN_MAX_KEY_GAP_MS ? event.key : buffer + event.key;
