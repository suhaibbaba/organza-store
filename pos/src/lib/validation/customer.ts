...
// Mirrors admin/src/lib/validation/order-form.ts: the same order, taken at
// the counter instead of at a desk. Field messages are backend error codes
// (CLAUDE.md rule 12), so one translator renders both client- and
...
export const orderCustomerFormSchema = z
  .object({
    ...
    address: z.string(),
    latitude: z.string().refine((v) => isCoordinate(v, LATITUDE_MIN, LATITUDE_MAX), {
      message: ERROR_CODES.ORDER_LOCATION_INVALID,
    }),
    longitude: z.string().refine((v) => isCoordinate(v, LONGITUDE_MIN, LONGITUDE_MAX), {
      message: ERROR_CODES.ORDER_LOCATION_INVALID,
    }),
    note: z.string(),
  })
