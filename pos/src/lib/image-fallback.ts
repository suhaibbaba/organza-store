 ...
 * again. Without this, every re-render asks for it afresh — and a search
 * result list re-renders on every keystroke, every refetch and every scroll
 * that remounts a row, so one deleted photo turns into a steady drip of
 * failing requests and a thumbnail that flickers from placeholder to broken
 * and back. Remembered here instead, so the second render onwards draws the
 * placeholder immediately and asks for nothing.
 *
 ...
 * so they resolve against the API origin rather than this app's. Anything
