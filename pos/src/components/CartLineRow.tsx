        ...
        <div className="flex-1 p-3 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="flex-1 min-w-0">
              ...
              {variantTitle && (
                <Badge variant="muted" size="sm" className="mt-0.5">{variantTitle}</Badge>
              )}
            </div>
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <span className="font-bold text-sm text-foreground">{format(lineTotal)}</span>
              <Button variant="ghost" size="icon-sm" onClick={() => onRemove(item.variant.id)} className="text-destructive hover:text-destructive hover:bg-destructive/10">
                <IconTrash size={14} />
              </Button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">{format(item.unitPrice)} {t('pos.unitPrice')}</span>
            ...
            <div className="flex items-center gap-1 ml-auto">
              <Input
                ...
                onChange={(e) => onDiscount(item.variant.id, Number(e.currentTarget.value) || 0, item.lineDiscountType)}
                min={0}
              ...
              <div className="flex rounded-lg border border-border overflow-hidden text-xs h-8">
                {(['fixed', 'percent'] as const).map((v) => (
                  <button
                    key={v}
                    onClick={() => onDiscount(item.variant.id, item.lineDiscount, v)}
