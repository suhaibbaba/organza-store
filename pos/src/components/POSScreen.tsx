...
export function POSScreen({ onLogout, onLanguageToggle }: Props) {
      ...
      const items = cart.map((l) => {
        ...
        return {
          ...
          unit_price: Math.round(
            Math.max(0, (gross - disc) / l.quantity) * 100,
          ),
        };
      });
  ...
  const summaryProps = {
    ...
    onCartDiscountChange: (v: number, ty: "fixed" | "percent") => {
  ...
  return (
    <TooltipProvider>
      <div className="flex flex-col h-screen overflow-hidden">
        {/* ── Header ── */}
        <header
          className="h-16 flex-shrink-0 flex items-center px-3 sm:px-5 justify-between gap-3"
          style={{
            background: "#235C63",
        ...
        >
          ...
          <div className="flex-1 flex items-center justify-center gap-2 max-w-[500px]">
            <Select
              ...
              onValueChange={(v) => {
                setRegionId(v);
                saveSettings({ ...loadSettings(), regionId: v });
              }}
            >
              <SelectTrigger
                size="sm"
                className="border-white/20 text-white bg-white/10 hover:bg-white/15 focus:ring-white/30"
                style={{ width: isMobile ? 130 : 190, color: "white" }}
              >
            ...
            {!isMobile && salesChannels.length > 1 && (
              <Select
                ...
                onValueChange={(v) => {
                  setSalesChannelId(v);
                  saveSettings({ ...loadSettings(), salesChannelId: v });
                }}
              >
                <SelectTrigger
                  size="sm"
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="text-white/80 hover:text-white hover:bg-white/15 flex-shrink-0"
              >
                <IconSettings size={20} />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>{t("app.options")}</DropdownMenuLabel>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className="aspect-square border-primary/30 text-primary hover:bg-primary/10"
                    >
              ...
              {cart.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center gap-4 py-10">
                  ...
                  <div className="text-center">
                    <p className="font-bold text-lg text-muted-foreground/70">
                      {t("pos.emptyCart")}
                    </p>
