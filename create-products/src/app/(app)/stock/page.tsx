...
import { Search, ChevronUp, ChevronDown, AlertTriangle, Package } from 'lucide-react'
...
export default function StockPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const allProducts: Product[] = [...p1.products, ...p2.products.filter(p => !p1.products.find(x => x.id === p.id))]
      ...
      for (const p of allProducts) {
        for (const v of (p.variants || [])) {
          flat.push({
            ...
            variantId: v.id,
            variantTitle: v.options?.map(o => o.value).join(' / ') || v.title || 'Default',
            sku: v.sku,
  ...
  const adjust = (variantId: string, delta: number) => {
    setRows(prev => prev.map(r =>
      r.variantId === variantId
        ? { ...r, pendingStock: Math.max(0, (r.pendingStock ?? r.stock) + delta) }
        : r
    ))
  }
    ...
    try {
      ...
      setRows(prev => prev.map(r =>
        r.variantId === row.variantId
          ? { ...r, stock: r.pendingStock!, pendingStock: null, saving: false }
          : r
      ))
  ...
  const filtered = rows.filter(r => {
    const matchQ = !q || r.productTitle.toLowerCase().includes(q.toLowerCase()) || r.sku?.includes(q)
      ...
      <div className="bg-white border-b border-slate-100 px-4 pt-14 pb-4 sticky top-0 z-20">
        ...
        <div className="relative mb-3">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search by name or SKU…"
            className="w-full pl-9 pr-4 py-2.5 bg-slate-100 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all" />
        </div>
        {/* Filter tabs */}
        <div className="flex bg-slate-100 rounded-xl p-1">
          {([['all', 'All'], ['low', `Low Stock ≤${LOW_STOCK}`]] as const).map(([val, label]) => (
            <button key={val} onClick={() => setFilter(val)}
              className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                filter === val ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500'
              }`}
            >
              {label}
            </button>
      ...
      <div className="flex-1 overflow-y-auto pb-nav px-4 py-3 space-y-2">
        {loading ? (
          ...
          filtered.map(row => {
            ...
            const isDirty = row.pendingStock !== null && row.pendingStock !== row.stock
            const isLow = row.stock <= LOW_STOCK

            return (
              <div key={row.variantId} className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${isLow ? 'border-amber-200' : 'border-slate-100'}`}>
                <div className="flex items-center gap-3 p-3">
                  {/* Thumbnail */}
                  <div className="w-12 h-12 rounded-xl bg-slate-100 flex-shrink-0 overflow-hidden">
                    {row.productThumb
                      ? <img src={row.productThumb} alt="" className="w-full h-full object-cover" />
                      : <div className="w-full h-full flex items-center justify-center"><Package className="w-5 h-5 text-slate-300" /></div>
                    }
                  </div>
                  {/* Info */}
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-slate-900 truncate">{row.productTitle}</p>
                    <p className="text-[11px] text-slate-500 truncate">{row.variantTitle}</p>
                    {row.sku && <p className="text-[10px] text-slate-400">SKU: {row.sku}</p>}
                    {isLow && (
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => adjust(row.variantId, -1)} disabled={row.saving}
                    ...
                    <input
                      ...
                      min="0"
                      value={displayStock}
                      onChange={e => setPending(row.variantId, e.target.value)}
                      className={`w-12 text-center text-sm font-bold border-2 rounded-lg py-1.5 focus:outline-none transition-colors ${
                        isDirty ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-slate-200 text-slate-900 bg-white'
                      }`}
                    />
                    <button onClick={() => adjust(row.variantId, 1)} disabled={row.saving}
                      className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-600 active:scale-90 transition-all">
                ...
                {/* Save bar */}
                {isDirty && (
                  <div className="flex border-t border-indigo-100 bg-indigo-50">
                    <button onClick={() => setRows(prev => prev.map(r => r.variantId === row.variantId ? { ...r, pendingStock: null } : r))}
                      className="flex-1 py-2 text-xs font-medium text-slate-500">
                      Cancel
                    </button>
                    <button onClick={() => saveStock(row)} disabled={row.saving}
                      className="flex-1 py-2 text-xs font-bold text-indigo-600 border-l border-indigo-100 flex items-center justify-center gap-1.5">
